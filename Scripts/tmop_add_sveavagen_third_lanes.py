...
Read the Output Log, then change APPLY_CHANGES to True and run it again.
Save the level only after visually checking the generated splines.
"""
...
def offset_left(points, distance_cm):
    """Offset a directed polyline to its left while retaining every Z value."""
    shifted = []
    for index, point in enumerate(points):
        ...
        left = unreal.Vector(-float(forward.y), float(forward.x), 0.0)
        shifted.append(point + left * distance_cm)
    return shifted
...
def spawn_lane_actor(new_id, points, template=None, crossing=False):
    ...
    set_prop(actor, "lane_i_d", unreal.Name(new_id))
    ...
    set_prop(lane, "lane_index_from_right", 1 if crossing else 3)
...
def r3_id(r2_id):
    if SVEA_ROAD_PATTERN.match(r2_id) and r2_id.endswith("_R2"):
        return r2_id[:-2] + "R3"
    return r2_id
...
r2_lanes = {
    ...
    and lane_id(lane).endswith("_R2")
}

expected_r2 = [
    "SVEAVAGEN{}_{}{}_R2".format(direction, "0" * (3 - len(str(section))), section)
    for direction in ("N", "S")
    for section in range(1, 8)
]
missing_r2 = [item for item in expected_r2 if item not in r2_lanes]
if missing_r2:
    raise RuntimeError(
        "The open level is missing required Sveavagen R2 lanes: {}".format(
            ", ".join(missing_r2)
        )
    )

planned_road_ids = [r3_id(item) for item in expected_r2]
existing_r3 = [item for item in planned_road_ids if item in by_id]

# Discover existing connector endpoints through the actual graph instead of
# trusting the connector name alone.
incoming_sources = {}
for source in lanes:
    for connection in prop(source, "next_lanes", []) or []:
        incoming_sources.setdefault(connection_target(connection), []).append(source)
...
for existing_connector_id, existing_connector in by_id.items():
    ...
    if turn != "LEFT":
        continue
    sources = incoming_sources.get(existing_connector_id, [])
    targets = [
        by_id.get(connection_target(connection))
        for connection in prop(existing_connector, "next_lanes", []) or []
    ]
    targets = [target for target in targets if target is not None]
    for source in sources:
        for target in targets:
            source_id = lane_id(source)
            target_id = lane_id(target)
            source_is_svea_r2 = source_id in r2_lanes
            target_is_svea_r2 = target_id in r2_lanes
            if source_is_svea_r2 == target_is_svea_r2:
                continue
            new_source_id = r3_id(source_id)
            new_target_id = r3_id(target_id)
            adjacent_plans.append((new_source_id, new_target_id, turn))
...
unreal.log("TMOP R3 dry-run={} lane_width_cm={:.1f}".format(
    not APPLY_CHANGES, LANE_WIDTH_CM))
...
if not APPLY_CHANGES:
    unreal.log_warning(
        "TMOP R3 DRY RUN ONLY. Set APPLY_CHANGES = True after checking the log."
    )
else:
    created_lanes = 0
    ...
    with unreal.ScopedEditorTransaction("Add Sveavagen R3 lanes and connectors"):
        ...
        for source_id in expected_r2:
            new_id = r3_id(source_id)
            if new_id in by_id:
                continue
            template = r2_lanes[source_id]
            points = offset_left(world_points(template), LANE_WIDTH_CM)
            _, new_lane = spawn_lane_actor(new_id, points, template, False)
            by_id[new_id] = new_lane
            created_lanes += 1

        # R1/R2/R3 neighbour relationships. R1 remains curbside; R3 is median-side.
        for direction in ("N", "S"):
            for section in range(1, 8):
                ...
                set_prop(r1, "lane_index_from_right", 1)
                set_prop(r1, "right_neighbor_lane_id", unreal.Name(""))
                set_prop(r1, "left_neighbor_lane_id", unreal.Name(stem + "_R2"))
                set_prop(r2, "lane_index_from_right", 2)
                set_prop(r2, "right_neighbor_lane_id", unreal.Name(stem + "_R1"))
                set_prop(r2, "left_neighbor_lane_id", unreal.Name(stem + "_R3"))
                set_prop(r3, "lane_index_from_right", 3)
                set_prop(r3, "right_neighbor_lane_id", unreal.Name(stem + "_R2"))
                set_prop(r3, "left_neighbor_lane_id", unreal.Name(""))

        # Create both the through-road connectors and median-side left turns.
