...
# (run_id, route_asset, first_stop_hms, driver_entity_id)
RUNS = [
...
def make_run(run_id, route_name, first_stop_hms, driver_id, bus_class):
    ...
    run.set_editor_property(
        "exact_start_time",
        tmop_time(hms_minus_seconds(first_stop_hms, SPAWN_LEAD_SECONDS)))
    ...
    run.set_editor_property(
        "source_reference",
        "SL weekday timetable 1986; route histories; reconstructed stop passage time")
    return run
