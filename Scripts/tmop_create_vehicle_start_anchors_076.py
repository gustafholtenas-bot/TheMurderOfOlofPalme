...
existing = {}
for actor in actors:
    if not actor.is_a(anchor_class):
        continue
    try:
...
for item in ANCHORS:
    ...
    rotation = unreal.Rotator(
        rotation_data["pitch"], rotation_data["yaw"], rotation_data["roll"]
    )
    ...
    prop(actor, "b_can_be_used_for_routing", True)
    prop(actor, "b_hard_historical_anchor", True)
