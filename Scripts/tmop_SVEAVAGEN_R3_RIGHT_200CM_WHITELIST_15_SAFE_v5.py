...
# Exact right-turn connectors from TMOP_Lanes_Anchors(6).json. The live level
# can contain older repair connectors, so automatic discovery is unsafe here.
RIGHT_TURN_R1_ENDPOINTS = [
    ("ADOLFFREDRIKSKYRKOGE_003_R1", "SVEAVAGENS_004_R1"),
    ("ADOLFFREDRIKSKYRKOGW_001_R1", "SVEAVAGENN_005_R1"),
    ("APELBERGSGATANE_002_R1", "SVEAVAGENS_006_R1"),
    ("KAMMAKAREGATANW_002_R1", "SVEAVAGENN_006_R1"),
    ("KUNGSGATANE_002_R1", "SVEAVAGENS_007_R1"),
    ("KUNGSGATANW_003_R1", "SVEAVAGENN_002_R1"),
    ("SVEAVAGENN_001_R1", "KUNGSGATANE_003_R1"),
    ("SVEAVAGENN_002_R1", "APELBERGSGATANE_003_R1"),
    ("SVEAVAGENN_006_R1", "TEGNERGATANE_004_R1"),
    ("SVEAVAGENS_001_R1", "TEGNERGATANW_005_R1"),
    ("SVEAVAGENS_002_R1", "KAMMAKAREGATANW_003_R1"),
    ("SVEAVAGENS_004_R1", "TUNNELGATANW_001_R1"),
    ("SVEAVAGENS_006_R1", "KUNGSGATANW_004_R1"),
    ("TEGNERGATANE_003_R1", "SVEAVAGENS_002_R1"),
    ("TUNNELGATANE_003_R1", "SVEAVAGENS_005_R1"),
]
...
adjacent_plans = []
for source_id, target_id in RIGHT_TURN_R1_ENDPOINTS:
    missing = [item for item in (source_id, target_id) if item not in by_id]
    if missing:
        raise RuntimeError(
            "Safety stop: required right-turn endpoint is missing: {}. "
            "No changes were made.".format(", ".join(missing))
        )
    source_is_svea_r1 = source_id in r1_lanes
    target_is_svea_r1 = target_id in r1_lanes
    if source_is_svea_r1 == target_is_svea_r1:
        raise RuntimeError(
            "Safety stop: invalid R1 whitelist pair {} -> {}. No changes "
            "were made.".format(source_id, target_id)
        )
    adjacent_plans.append((r3_id(source_id), r3_id(target_id), "RIGHT"))
