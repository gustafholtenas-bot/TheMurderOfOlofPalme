...
void STMOPControlsPanel::Rebuild()
{
    ...
    for (int32 Index = 0; Index < 4; ++Index)
        ...
        [ SNew(SButton)
          .Text(FText::FromString(FString::Printf(TEXT("SPELARE %d%s"), Index + 1,
              Index == SelectedPlayer ? TEXT(" ✓") : TEXT(""))))
          .OnClicked(this, &STMOPControlsPanel::SelectPlayer, Index) ];
    Body->AddSlot().AutoHeight().Padding(2.0f, 8.0f)[Players];

    Body->AddSlot().AutoHeight().Padding(2.0f, 4.0f)
    [ SNew(SButton).Text_Lambda([this]() { return DeviceText(); })
    ...
    AddActionSection(FText::FromString(TEXT("TILL FOTS")), {
    ...
    AddActionSection(FText::FromString(TEXT("FORDON")), {
    ...
    [ SNew(SButton).Text(FText::FromString(TEXT("ÅTERSTÄLL SPELARENS STANDARDKONTROLLER")))
...
FReply STMOPControlsPanel::CycleDevice()
{
    ...
    Rebuild();
    if (!bChanged && Status.IsValid()) Status->SetText(Error);
    return FReply::Handled();
}
    ...
    const bool bSecondary)
{
    ...
    {
        if (Status.IsValid()) Status->SetText(FText::FromString(TEXT("En annan ombindning pågår. Slutför den först.")));
        return FReply::Handled();
    }
    ...
    if (Status.IsValid()) Status->SetText(FText::Format(FText::FromString(
        TEXT("Tryck ny knapp för {0}. Escape avbryter.")),
...
void STMOPControlsPanel::CommitBinding(const FKey Key)
{
    ...
    if (Status.IsValid()) Status->SetText(bOk
        ? FText::Format(FText::FromString(TEXT("Sparad: {0}")), Key.GetDisplayName(false))
        : Error);
    if (bOk)
    {
        PendingAction.Reset();
...
FReply STMOPControlsPanel::OnKeyDown(const FGeometry& Geometry, const FKeyEvent& Event)
{
    if (PendingAction.IsSet())
    {
        if (Event.GetKey() == EKeys::Escape)
        {
            ...
            if (Status.IsValid()) Status->SetText(FText::FromString(TEXT("Ombindning avbruten.")));
        }
...
FReply STMOPControlsPanel::ResetSelectedPlayer()
{
    ...
    PendingAction.Reset();
    Rebuild();
    if (!bReset && Status.IsValid()) Status->SetText(Error);
    return FReply::Handled();
}
...
{
    if (!Controls.IsValid()) return FText::FromString(TEXT("Styrning saknas"));
    switch (Controls->GetProfile(SelectedPlayer).Device)
    {
    case ETMOPControlDevice::KeyboardMouse: return FText::FromString(TEXT("ENHET: TANGENTBORD + MUS"));
    case ETMOPControlDevice::KeyboardOnly: return FText::FromString(TEXT("ENHET: DELAT TANGENTBORD"));
    default: return FText::FromString(TEXT("ENHET: HANDKONTROLL"));
    }
}
    ...
    const bool bSecondary) const
{
    ...
    return Key.IsValid() ? Key.GetDisplayName(false)
        : FText::FromString(bSecondary ? TEXT("+ Lägg till") : TEXT("Ej bunden"));
}
